        ...
        <form action="{{ route('products.update', $product) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="border p-2 rounded w-full" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $product->price) }}" class="border p-2 rounded w-full" required>
                    @error('price')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" class="border p-2 rounded w-full" required>
                    @error('stock')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="bg-blue-500 text-white p-2 rounded hover:bg-blue-600">Actualizar</button>
            </div>
        </form>
    </div>
@endsection